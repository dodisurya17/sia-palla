<x-app-layout :title="'Manajemen User'">
    <div class="bg-white border rounded-lg" x-data="{ deleteModal: false, deleteFormId: null, deleteName: '' }">
        <div class="flex items-center justify-between p-5 border-b">
            <form method="GET" action="{{ route('user.index') }}" class="flex-1 max-w-xs" id="searchForm">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y">
                ...
                <tr>
                    ...
                    <td class="px-5 py-3 text-right">
                        <div class="flex items-center justify-end space-x-3">
                            ...
                            {{-- Hapus --}}
                            <form id="delete-form-{{ $user->id }}" action="{{ route('user.destroy', $user) }}" method="POST" class="hidden">
                                @csrf @method('DELETE')
                            </form>
                            @if ($user->id !== auth()->id())
                            <button type="button"
                                @click="deleteModal = true; deleteFormId = 'delete-form-{{ $user->id }}'; deleteName = '{{ $user->name }}'"
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-5 py-6 text-center text-gray-400">Belum ada data user.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div x-show="deleteModal" x-cloak
            ...
            <div x-show="deleteModal" x-transition
                ...
                <h3 class="text-base font-semibold text-slate-800 mb-1">Hapus User</h3>
