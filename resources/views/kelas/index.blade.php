        ...
        <div class="bg-white rounded-2xl shadow-sm border overflow-hidden">
            ...
            <div class="p-6 border-b border-gray-100 flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-slate-800">Data Kelas</h2>
                        <p class="text-sm text-slate-500">Kelola daftar kelas yang tersedia</p>
                    </div>
                </div>
            ...
            <div class="px-6 py-4 border-b border-gray-100">
                <form method="GET" action="{{ route('kelas.index') }}" class="max-w-md">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50/60 transition">
                            ...
                            <td class="px-6 py-4 text-slate-700 font-medium">{{ $k->tingkat ?? '-' }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $k->wali_kelas ?? '-' }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $k->siswa_count }} siswa</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <button type="button"
                                        @click="openShowModal('{{ route('kelas.show', $k) }}')"
                                    ...
                                    <button type="button"
                                        @click="deleteFormId = 'delete-form-{{ $k->id }}'; deleteName = '{{ $k->nama_kelas }}'"
                                    ...
                                    <form id="delete-form-{{ $k->id }}" method="POST"
                                        action="{{ route('kelas.destroy', $k) }}" class="hidden">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </div>
        ...
        <template x-teleport="body">
            <div x-show="deleteFormId !== null" x-cloak
                ...
                <div x-show="deleteFormId !== null" x-transition
                    ...
                    <h3 class="text-lg font-semibold text-slate-800 mb-1">Hapus Kelas?</h3>
