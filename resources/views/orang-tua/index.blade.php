<x-app-layout :title="'Data Orang Tua'">
    <div class="bg-white border rounded-lg" x-data="{ deleteModal: false, deleteFormId: null, deleteName: '' }">
        <div class="flex items-center justify-between p-5 border-b">
            <form method="GET" action="{{ route('orang-tua.index') }}" class="flex-1 max-w-xs" id="searchForm">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y">
                ...
                <tr>
                    <td class="px-5 py-3">{{ $orangTua->nama }}</td>
                    <td class="px-5 py-3">{{ $orangTua->no_hp ?? '-' }}</td>
                    <td class="px-5 py-3">{{ $orangTua->pekerjaan ?? '-' }}</td>
                    <td class="px-5 py-3">{{ $orangTua->siswa_count }}</td>
                    <td class="px-5 py-3 text-right">
                        <div class="flex items-center justify-end space-x-3">
                            {{-- Lihat --}}
                            <a href="{{ route('orang-tua.show', $orangTua) }}"
                            ...
                            {{-- Hapus --}}
                            <form id="delete-form-{{ $orangTua->id }}" action="{{ route('orang-tua.destroy', $orangTua) }}" method="POST" class="hidden">
                                @csrf @method('DELETE')
                            </form>
                            <button type="button"
                                @click="deleteModal = true; deleteFormId = 'delete-form-{{ $orangTua->id }}'; deleteName = '{{ $orangTua->nama }}'"
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-5 py-6 text-center text-gray-400">Belum ada data orang tua.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div x-show="deleteModal" x-cloak
            ...
            <div x-show="deleteModal" x-transition
                ...
                <h3 class="text-base font-semibold text-slate-800 mb-1">Hapus Data Orang Tua</h3>
