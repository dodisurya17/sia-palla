<x-app-layout>
    <x-slot name="title">Edit Nilai Ekstrakurikuler</x-slot>
    ...
    <div class="bg-white rounded-2xl shadow-sm border overflow-hidden max-w-3xl mx-auto">
        ...
        <div class="p-6 border-b border-gray-100 flex items-center gap-4">
            ...
            <div>
                <h2 class="text-lg font-semibold text-slate-800">Edit Nilai Ekstrakurikuler</h2>
                <p class="text-sm text-slate-500">{{ $nilai->siswa->nama ?? '-' }} &middot; {{ $nilai->ekstrakurikuler->nama_ekstrakurikuler ?? '-' }}</p>
            </div>
        </div>

        <form method="POST" action="{{ route('nilai-ekstrakurikuler.update', $nilai) }}" class="p-6">
            @csrf
            @method('PUT')
