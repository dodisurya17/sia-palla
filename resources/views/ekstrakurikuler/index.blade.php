        ...
        <div class="bg-white rounded-2xl shadow-sm border overflow-hidden">
            ...
            <div class="p-6 border-b border-gray-100 flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-lg font-semibold text-slate-800">Data Ekstrakurikuler</h2>
                        <p class="text-sm text-slate-500">Kelola daftar kegiatan ekstrakurikuler sekolah</p>
                    </div>
                </div>
            ...
            <div class="px-6 py-4 border-b border-gray-100">
                <form method="GET" action="{{ route('ekstrakurikuler.index') }}" class="max-w-md">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 border-b text-left text-slate-500 uppercase text-xs tracking-wider">
                            <th class="px-6 py-4 font-medium">Nama Ekstrakurikuler</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50/60 transition">
                            <td class="px-6 py-4 text-slate-700 font-medium">{{ $ekskul->nama_ekstrakurikuler }}</td>
                            <td class="px-6 py-4">
                                @if ($ekskul->jenis === 'wajib')
                            ...
                            <td class="px-6 py-4 text-slate-600">{{ $ekskul->pembina ?? '-' }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $ekskul->jadwal ?? '-' }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <button type="button"
                                        @click="openShowModal('{{ route('ekstrakurikuler.show', $ekskul) }}')"
                                    ...
                                    <a href="{{ route('ekstrakurikuler.edit', $ekskul) }}"
                                    ...
                                    <button type="button"
                                        @click="deleteFormId = 'delete-form-{{ $ekskul->id }}'; deleteName = '{{ $ekskul->nama_ekstrakurikuler }}'"
                                    ...
                                    <form id="delete-form-{{ $ekskul->id }}" method="POST"
                                        action="{{ route('ekstrakurikuler.destroy', $ekskul) }}" class="hidden">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </div>
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center text-slate-400">
                                <svg class="w-10 h-10 mx-auto mb-3 text-slate-300" fill="none" stroke="currentColor"
                                    ...
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M12 15a3 3 0 100-6 3 3 0 000 6zM12 3v2m0 14v2m9-9h-2M5 12H3m15.364-6.364l-1.414 1.414M7.05 16.95l-1.414 1.414m0-12.728l1.414 1.414M16.95 16.95l1.414 1.414" />
                                </svg>
        ...
        <template x-teleport="body">
            <div x-show="deleteFormId !== null" x-cloak
                ...
                <div x-show="deleteFormId !== null" x-transition
                    ...
                    <h3 class="text-lg font-semibold text-slate-800 mb-1">Hapus Ekstrakurikuler?</h3>
